...
class TestPublicUsers:
    ...
    async def test_create_user(self, db_session, async_public_client):
        ...
        created_user = await db_session.scalar(select(User).where(User.id == 1))
...
class TestAuthenticatedUser:
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_with_refresh_token_type(
        self, async_public_client, mocker
    ):
        mocker.patch("app.backend.redis.token_in_blacklist", return_value=None)
        token = await generate_token("refresh", 2, "test")

        headers = {"Authorization": f"Bearer {token}"}

        response = await async_public_client.get("user/me", headers=headers)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.json() == {
    ...
    async def test_change_password(self, async_client, test_user):
        response = await async_client.put(
            "user/", json={"password": "test123", "new_password": "test1234578"}
        )
    ...
    async def test_change_password_invalid_current_password(
        ...
        response = await async_client.put(
            "user/", json={"password": "test", "new_password": "test12345"}
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.json() == {"detail": "Error on password change"}

    @pytest.mark.asyncio
    async def test_change_user_name_success(self, async_client, test_user, db_session):
    ...
    async def test_create_user_by_authenticated_user(
        self, db_session, async_client, mocker
    ):
        mocker.patch("app.backend.redis.token_in_blacklist", return_value=None)
        token = await generate_token("access", 2, "test")
        ...
        response = await async_client.post("user/", json=payload, headers=headers)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json() == {
            "detail": "You've already registered, registration is not available"
        }
        assert await db_session.scalar(select(exists().where(User.id == 1))) is False
    ...
    @pytest.mark.asyncio
    async def test_change_user_role(self, async_client, test_user, db_session):
        response = await async_client.put("user/role/manager?user_id=1")
        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert response.json() == {
            "detail": "You do not have permission to perform this action"
        }
        updated_user = await db_session.scalar(select(User).where(User.id == 1))
...
class TestAuthenticatedAdminUser:
    ...
    @pytest.mark.asyncio
    async def test_delete_user_not_exist(self, async_client_admin):
        response = await async_client_admin.delete("/user/1")
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json() == {"detail": "User not found"}

    @pytest.mark.asyncio
    async def test_change_user_role(self, async_client_admin, test_user, db_session):
