...
async def test_get_my_meetings(async_client, test_meetings, db_session):
    meets = await db_session.scalars(
        select(Meeting).join(user_meeting).where(
            and_(
                user_meeting.c.user_id == 1,
                Meeting.date >= datetime.now().date()
            )
        ).options(selectinload(Meeting.participants))
    )
    meets_out = [MeetingOut.model_validate(m).model_dump(mode="json")
                 for m in meets.all()]
...
async def test_create_meeting(
        async_client, test_user, test_user2,
        test_user3, db_session):
    payload = {
        'title': 'New meeting',
...
@pytest.mark.parametrize('date', [
        ...
        '2025-08-31T14:00:00Z',
        '2025-08-31T15:29:59Z'
    ])
@time_machine.travel("2025-08-25 10:00:00")
@pytest.mark.asyncio
async def test_create_overlapping_meeting(
        async_client, date, test_user, test_user2,
        test_user3, test_meetings, db_session):
    payload = {
        'title': 'New meeting',
        'description': 'Description',
        'date': date,
    ...
    assert response.json() == {
        'detail': 'You have another meeting at 2025-08-31 14:30:00'
    }
...
async def test_create_unknown_participant(
    ...
    assert response.json() == {
        'detail': "There's no user with 99 id"
    }
    assert await db_session.scalar(select(exists().where(
        Meeting.title == payload['title']
    ))) is False
...
async def test_create_meeting_unauthenticated(
        async_public_client, test_user, test_user2,
        test_user3, test_meetings, db_session):
    payload = {
        'title': 'New meeting',
        'description': 'Description',
...
async def test_delete_meeting(
        async_client, test_meetings, db_session):
...
@pytest.mark.asyncio
async def test_delete_meeting_unathenticated(
        async_public_client, test_meetings):
    response = await async_public_client.delete('meeting/1')
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


@pytest.mark.asyncio
async def test_delete_meeting_not_exists(
        async_client, test_meetings):
